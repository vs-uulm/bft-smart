...
public class NettyClientServerCommunicationSystemServerSide extends SimpleChannelHandler implements CommunicationSystemServerSide  {
    ...
    public NettyClientServerCommunicationSystemServerSide(TOMConfiguration conf) {
        try {            
            ...
            Executor pool = createThreadPool();
            ...
            do {
            	try{
					serverchannel = bootstrap.bind(new InetSocketAddress(conf
							.getHost(conf.getProcessId()), conf.getPort(conf
							.getProcessId())));
					bound = true;
            		...
            		log.info("Failed to bind to channel, retrying in 3 seconds)");
        			try {
						Thread.sleep(3);
					} catch (InterruptedException e) {
						// got interrupted while waiting to rebind
						log.warning("Interrupted while waiting to bind to server"
								+ "socket for client connections, exiting");
						break;
					}
            	}
    ...
    public static ExecutorService createThreadPool(){
        ...
        ThreadFactory tfactory = new ThreadFactory() {
            ...
            public Thread newThread(Runnable r) {
                ...
                if (System.getProperty("navigators.smart.communication."
						+ "client.netty.threadpriority") != null) {
                    try {
                        Integer.parseInt(System.getProperty("navigators."
								+ "smart.communication.client.netty"
								+ ".threadpriority"));
        ...
        return Executors.newCachedThreadPool(tfactory);
    }

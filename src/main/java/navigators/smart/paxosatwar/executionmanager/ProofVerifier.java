...
public class ProofVerifier {
	 ...
	 * @return The value considered to be good, if any, and its nextLeader. If
	...
	/**
	 * Gets the initial proposer for a value that was not decided in the first 
	 * round. There must be at least 2f+1 proofs because otherwise the
	 * proposal wont be accepted.
	 * 
	 * @param val The value that was decided upon
	 * @param proofs The proofs from at lest 2f+1 acceptors
	 * 
	 * @return The proposer or -1 if none was found;
	 */
	public Integer getInitialProposer(Propose p){
		if(p.getProof() == null){
			return -1;
		}
		
		CollectProof[] proofs = p.getProof().getProofs();
		// Should be obsolete to check this but anyways...
		if(proofs == null){
			return -1;
		}
		...
		for (int i = 0; i < rounds + 1; i++) {
			if(log.isLoggable(Level.FINE)){
				log.log(Level.FINE,"Checking round {0} for a potentially "
					+ "good proposer",new Object[]{i});
			}
			RoundInfo info = buildRoundInfo(proofs, i);
			if(info.poss.contains(new ByteWrapper(p.value))){
				if(log.isLoggable(Level.FINE)){
					log.log(Level.FINE,"Round {0} has the value in poss: "
							+ "selecting good proposer {1}",
							new Object[]{i,info.proposer});
				}
	 ...
	 * 
	 */
	public boolean isGood(byte[] value, CollectProof[] proofs, Integer r) {
		ByteWrapper w = new ByteWrapper(value);
		List<RoundInfo> infos = buildInfos(proofs);

		/*
		 * condition G2 in Paxos At War beginning from a proposed value in round
		 * 0, iteratively trying higher rounds if this fails.
		 */
		for (RoundInfo s : infos) {
	...
	private boolean checkGoodG2(ByteWrapper val, RoundInfo info, Integer r,
		...
		// val was not even contained in acc
		log.fine("Received proposal is not contained in acc");
		return false;
	}
	 ...
	 *         good value.
	 */
	private RoundInfo buildRoundInfo(CollectProof[] col_proofsin, int round) {

		// Copy collectproofs to be able to null values
		CollectProof[] proofs = Arrays.copyOf(col_proofsin, col_proofsin.length);
		Set<ByteWrapper> acc = new HashSet<ByteWrapper>();
		Set<ByteWrapper> poss = new HashSet<ByteWrapper>();
		Set<ByteWrapper> vals = getProposedValues(col_proofsin, round);
		...
		for (int i = 0; i < proofs.length; i++) {
			...
			if (w != null && !acc.contains(w) && !poss.contains(w)) {
				...
				// no null check needed for weakcount as the enclosing if does this
				int weakcount = 1; 
				int strongcount = s != null ? 1 : 0;
				// Check all other proofs
				for (int j = i + 1; j < proofs.length; j++) {

					// Check other proofs for the same w
					if (proofs[j] != null) {
						// weak and strong value of currentt other acceptor
						ByteWrapper cw = getWeakProof(proofs[j], round); 
						ByteWrapper cs = getStrongProof(proofs[j], round); 
						
						// The other proof is also weakly accepted and the
						// values are equal
						if (cw != null && w.equals(cw)) {
							weakcount++;
							proposers[proofs[j].getProofs().get(round).getProposer()]++;
							/*
							 * Eliminate the proof of this other acceptor as it 
							 * was already counted
							 */
							proofs[j] = null; 
						}
						//Check if the value was strongly accepted
						if (cs != null && cs.equals(w)) {
							strongcount++;
						}
					}
				}
				...
				/*
				 * If we found a possibly strongly accepted value, add it to 
				 * poss and honor out the leader that proposed it.
				 */

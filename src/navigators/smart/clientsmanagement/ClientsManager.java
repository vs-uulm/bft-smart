...
/**
 * Holds a list of all currently active Clients and their pending Requests. The contents of this object differ across replicas, as clients may send
...
public class ClientsManager {
	 ...
	 * Registers a
	 *
	 * @see ClientRequestListener at this manager
	...
	/**
	 * Get pending requests in a fair way (one request from each client queue until the max number of requests is gotten). The last client that was
	 * handled is stored to be really fair and have no side effekts when the batch size is below the number of clients.
	 *
	...
	public PendingRequests getPendingRequests() {
		...
		if (log.isLoggable(Level.FINER)) {
			log.log(Level.FINER, "Returning {0} messages, maxBatchsize: {1}", new Object[]{allReq.size(), conf.getMaxBatchSize()});
			printPending();
		}
		return allReq;
	}
	 ...
	 *
	 * @return true if the request is ok and is added to the pending messages for this client, false if there is some problem and the message was not
	 * accounted
	 */
	public boolean requestReceived(TOMMessage request, boolean fromClient, boolean storeMessage) {
		...
		if (conf.getUseControlFlow() != 0) {
			if (fromClient && (clientData.getAllPendingRequests() > conf.getUseControlFlow())) {
...
//	                || ((request.getSequence() > clientData.getLastMessageReceived()) && !fromClient)) {
	...
	/**
	 * Notifies the ClientsManager that the request was executed. It cleans all state for this request (e.g., removes it from the pending requests
	...
	/**
	 * Resets all client related information. All incoming client requests will be treated as new. This is used to reset replicas between tests.
	 */
